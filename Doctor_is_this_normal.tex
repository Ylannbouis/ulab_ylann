...
sidewalk. The image, which can be seen in \autoref{fig: sidewalk}, claims that the sidewalk crack resembles the form
...
\begin{equation}
    f(x)=a \cdot e^{-\frac{1}{2} \frac{\left( x - \mu \right)^2}{\sigma^2}}
\end{equation}
where $a$ is a scaling constant, $\mu$ is the mean and $\sigma$ is the standard deviation.
\section{Results}
After our data analysis, \autoref{fig: graph} reveals that a normal distribution fits our data well.
\begin{figure}[h]
    \centering
...
\begin{equation}
    \chi_{\nu}^2=\frac{1}{\nu} \sum \frac{\left( O - E\right)^2}{E} = 0.22
\end{equation}
