...
x_data = np.linspace(0, 3 * np.pi, 1000)
y_data = np.linspace(0, 3 * np.pi, 1000)
X, Y = np.meshgrid(x_data, y_data)

pattern = np.multiply(np.cos(X), np.sqrt(Y))
...
x = torch.abs(torch.randn(N, D_in)) * 3.1415
y = (x[:, 0].cos()*x[:, 1].sqrt()).unsqueeze(1)
